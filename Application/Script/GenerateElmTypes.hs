  ...
  where
    definitions =
        Simplification.simplifyDefinition
            -- Add Elm types here
            <$> jsonDefinitions @Widget <> jsonDefinitions @LocationJSON <> jsonDefinitions @RequestJSON
            <> jsonDefinitions @ChecklistJSON <> jsonDefinitions @ChecklistItemJSON <> jsonDefinitions @ChecklistHeaderJSON
